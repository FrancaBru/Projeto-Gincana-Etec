	...
	<head>
		...
		<script type="text/javascript">
			function validaform(){
				...
				var qtdMin = document.forms["formulario3"].qtdMin.value;
				if(isNaN(qtdMin)){
					alert("O campo Quantidade Mínima precisa ser preenchido corretamente!");
					return false;
				}

				var qtdMax = document.forms["formulario3"].qtdMax.value;
				if(isNaN(qtdMax)){
					alert("O campo Quantidade Máxima precisa ser preenchido corretamente!");
					return false;
				}

				var limite = document.forms["formulario3"].limite.value;
				if(isNaN(limite)){
					alert("O campo Limite precisa ser preenchido corretamente!");
					return false;
				}

				var pontos = document.forms["formulario3"].pontos.value;
				if(isNaN(pontos)){
					alert("O campo Pontos precisa ser preenchido corretamente!");
					return false;
				}

				var curso = document.forms["formulario3"].curso.selectIndex;
				if(curso == 0){
				...
				var sexo = document.forms["formulario3"].sexo.selectIndex;
				if(sexo == 0){
					sexo = 'T';
				}else if(sexo == 1){
					sexo = 'M';
				...
				var grupo = document.forms["formulario3"].grupo.value;
				var inRifa = document.forms["formulario3"].inRifa.value;
				var valorRifa = document.forms["formulario3"].valorRifa.value;
				
				var descricao = document.forms["formulario3"].descricao.value.length;
				if(descricao < 5){
	...
	?>
		...
		<main>
			...
			<div class="container droppedHover">
				<form name="formulario3" action="salvar_exclusao_atividade.php" method="POST">
					...
					<div class="campos_princ">
						...
						<div class="botao">
							<button class="btn btn-lg btn-primary btn-block" type="submit" onClick="validaform();">Excluir</button>
						</div>
